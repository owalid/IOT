...
function install_and_configure_argo_cd
{
    ...
    argocd admin initial-password -n argocd $ARGOCD_OPTS
    while [ ! $? -eq 0 ]
    do
    ...
    argocd login argocd.local:8080 $ARGOCD_OPTS
    message "Change the password of the admin account"
    argocd account update-password $ARGOCD_OPTS
}

function add_wil_app_to_argocd
{
    message "Create an app with argo cd."
    argocd app create wil-app --repo https://github.com/kibatche/oel-ayad-chbadad-iot-p3.git --path . --dest-server https://kubernetes.default.svc --dest-namespace dev $ARGOCD_OPTS
    message "Add automated sync policy to wil-app"
    argocd app set wil-app --sync-policy automated $ARGOCD_OPTS
    message "Syncing app"
