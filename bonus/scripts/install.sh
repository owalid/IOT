...
function create_cluster_and_namespaces
{
    ...
    k3d cluster create p3-iot -p "8080:80@loadbalancer" -p "8888:30080@agent:0" -p "7777:30081" --agents 3
...
function install_and_configure_argo_cd
{
    ...
    grep -xq "export ARGOCD_OPTS='--port-forward-namespace argocd --insecure --plaintext'" $HOME/.bashrc
    if [ ! $? -eq 0 ];then
        echo "export ARGOCD_OPTS='--port-forward-namespace argocd --insecure --plaintext'" >> $HOME/.bashrc
        source $HOME/.bashrc
    fi
...
function add_gitlab
{
    message "Create gitlab deployment"
...
kubectl get ns | grep 'gitlab'
check_exit_code "namespace gitlab"
message "Create namespace gitlab ok"

message "Install helm..."
curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash

message "Install gitlab from helm"
